"""
Entraînement Random Forest - SANS DATA LEAKAGE
Seulement features disponibles PENDANT la session
"""
...
print("="*70)
print("🌲 RANDOM FOREST - SANS DATA LEAKAGE")
print("="*70)
...
# ══════════════════════════════════════════════════════════════
# FEATURES SANS LEAKAGE (disponibles PENDANT la session)
# ══════════════════════════════════════════════════════════════
...
print(f"\n📋 Features utilisées (SANS leakage) : {len(FEATURES_CLEAN)}")
...
with open('models/rf_expandium_NO_LEAKAGE.pkl', 'wb') as f:
    pickle.dump(model, f)

print("   ✅ models/rf_expandium_NO_LEAKAGE.pkl")
...
metrics = {
    ...
    'note': 'NO DATA LEAKAGE - Only features available DURING session'
}

with open('models/rf_expandium_NO_LEAKAGE_metrics.json', 'w') as f:
    json.dump(metrics, f, indent=2)

print("   ✅ models/rf_expandium_NO_LEAKAGE_metrics.json")
...
fig.suptitle('Random Forest - SANS Data Leakage (Features Réalistes)', fontsize=16, fontweight='bold')
...
# 2. Importance
axes[0, 1].barh(range(len(feature_importance)), feature_importance['importance'], color='steelblue', edgecolor='black')
axes[0, 1].set_yticks(range(len(feature_importance)))
axes[0, 1].set_yticklabels(feature_importance['feature'], fontsize=9)
axes[0, 1].set_title('Importance Features (Sans Leakage)', fontsize=14, fontweight='bold')
...
viz_path = 'results/rf_expandium_NO_LEAKAGE_results.png'
...
print("\n" + "="*70)
print("✅ ENTRAÎNEMENT TERMINÉ - RÉSULTATS RÉALISTES")
print("="*70)

print(f"""
🌲 RANDOM FOREST - SANS DATA LEAKAGE :
   Features       : {len(FEATURES_CLEAN)} (disponibles PENDANT session)
...
📁 FICHIERS CRÉÉS :
   • models/rf_expandium_NO_LEAKAGE.pkl
   • models/rf_expandium_NO_LEAKAGE_metrics.json
   • results/rf_expandium_NO_LEAKAGE_results.png

💡 DIFFÉRENCE AVEC VERSION PRÉCÉDENTE :
   Avec leakage    : 99.7% accuracy (TROP parfait)
   Sans leakage    : {test_metrics[0]*100:.1f}% accuracy (RÉALISTE)
   
🎯 INTERPRÉTATION :
   Ces résultats sont RÉALISTES et UTILISABLES 
   pour un système de prédiction temps réel.

🚀 VOIR RÉSULTATS :
   start results\\rf_expandium_NO_LEAKAGE_results.png
""")
